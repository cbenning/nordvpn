#!/bin/bash
trap cleanup SIGTERM # https://www.ctl.io/developers/blog/post/gracefully-stopping-docker-containers/
...
killswitch() {
	...
	[[ -n ${NETWORK4} ]] && for net in ${NETWORK4//[;,]/ }; do route_create4 "${net}"; done
	[[ -n ${WHITELIST} ]] && for domain in ${WHITELIST//[;,]/ }; do whitelist_add "${domain}"; done
	...
	[[ -n ${NETWORK6} ]] && for net in ${NETWORK6//[;,]/ }; do route_create6 "${net}"; done
}

route_create4() { # Add a route back to your network, so that return traffic works
...
route_create6() { # Add a route back to your network, so that return traffic works
...
whitelist_add() { # Allow unsecured traffic for an specific domain
...
setupnordvpn() {
	...
	[[ -n ${NETWORK4} ]] && for net in ${NETWORK4//[;,]/ }; do nordvpn whitelist add subnet "${net}"; done
